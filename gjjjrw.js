const $ = new Env('顾家家居');
const Babycare = ($.isNode() ? JSON.parse(process.env.gjjj) : $.getjson("gjjj")) || [];
let Utils = undefined;
...
!(async () => {
    ...
    $.msg($.name, '签到结束', `🔔bjjj, 结束! 🕛 ${((performance.now() - start) / 1000).toFixed(3)} 秒`);
})().catch((e) => {$.log(e)}).finally(() => {$.done();});

let start = performance.now(); // 记录签到开始时间

async function main() {
    console.log('bjjj签到开始');
    Utils = await loadUtils();
    for (const item of bjjj) {
...
async function getCookie() {
    ...
    const index = bjjj.findIndex(e => e.authorization == newData.authorization);
    if (index !== -1) {
        if (bjjj[index].authorization == newData.authorization) {
