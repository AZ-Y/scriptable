...
async function getCk() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        const authorization = head['authorization'];
        if (authorization) {
            const ckVal = $.toStr({ authorization });
            $.setdata(ckVal, _key);
            $.msg($.name, 'Authorization Retrieved Successfully 🎉', ckVal);
        } else {
            $.msg($.name, '', '❌ Failed to Retrieve Authorization');
        }
    }
}
...
async function main() {
    if (CK_Val) {
        const { authorization } = CK_Val;
        $.authorization = authorization;
    } else {
        $.msg($.name, '', '❌ Please Retrieve Authorization First 🎉');
        return;
    }
    const code = await signIn(); // Sign-in
    if (code.includes('authorization')) { return; } // Stop if there's an error with the authorization
    await getSignMsg();
    await summary();
}
...
async function signIn() {
    ...
    const body = `{"signDate":"${new Date().toISOString()}","cId":"BLqo2nmmoPgGuJtFDWlUjRI2b1b"}`;
    const headers = {
        ...getHeaders(),
        "Version": "3.22.0",
    };
    const rest = { url, body, headers };
    const { code, data, message } = await httpRequest(rest);
    _msg += `Sign-in: ${message}`; //{"code":"fail","message":"You have already signed in, please do not repeat the operation!"}
    pushMsg(_msg);
    return code;
}
...
async function getSignMsg() {
    const url = `https://api.bckid.com.cn/operation/front/bonus/userSign/v3/getSignMsg`;
    const body = `{"year":"${new Date().getFullYear()}","month":"${new Date().getMonth() + 1}"}`;
    const headers = { ...getHeaders() };
    const rest = { url, body, headers };
    const { code, data, message } = await httpRequest(rest);
    let _msg;
    _msg = (code == 'success') ? `Cumulative Sign-in: ${data?.continuousSignDay} days` : '';
    pushMsg(_msg);
}

// Energy Body Summary
async function summary() {
    const url = `https://api.bckid.com.cn/operation/front/bonus/userSign/v3/summary`;
    const headers = { ...getHeaders() };
    const rest = { url, headers };
    const { code, data, message } = await httpRequest(rest);
    let _msg;
    _msg = (code == 'success') ? `Energy Body: ${parseFloat(data?.total)}` : ''; //{"code":"success","data":{"total":"2.00"},"message":"API call successful"}
    pushMsg(_msg);
}
...
function getHeaders() {
    return {
        ...
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148/ios/bckidApp"
    };
}
...
async function httpRequest(options) {
    try {
        ...
        const _http = [
            new Promise((_, reject) => setTimeout(() => reject(`⛔️ Request Timeout: ${options['url']}`), _timeout)),
...
!(async () => {
    if (typeof $request !== `undefined`) {
        ...
        if (!CK_Val) throw new Error('❌ Please Retrieve Authorization First 🎉');
        await main();
    }
})().catch((e) => $.messages.push(e.message || e) && $.logErr(e))
    .finally(async () => {
        await sendMsg($.messages.join('\n').trimStart().trimEnd()); // Push notification
        $.done();
    });
