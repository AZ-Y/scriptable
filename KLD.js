...
async function main() {
    ...
    for (let user of userList) {
        if (user.ckStatus) {
            ...
            taskall.push(await user.fetchScore()); // Add the score fetching task
            await $.wait(user.getRandomTime());
        } else {
            // 将ck过期消息存入消息数组
...
class UserInfo {
    ...
    async signin() {
        try {
            const url = `https://kld.kldww.com/capi/customer/sign/add`;
            const method = `GET`;
            const headers = {
                'Accept-Encoding': `gzip,compress,br,deflate`,
                'content-type': `application/json`,
                'Connection': `keep-alive`,
                'Referer': `https://servicewechat.com/wxe5e825e99f557570/265/page-frame.html`,
                'Host': `kld.kldww.com`,
                'User-Agent': `Mozilla/5.0 (iPhone; CPU iPhone OS 16_1_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.50(0x18003237) NetType/4G Language/zh_CN`,
    ...
    async fetchScore() {
        try {
            const url = `https://kld.kldww.com/capi/customer/account/score/mydetail`;
            const method = `GET`;
            const headers = {
                'Accept-Encoding': `gzip,compress,br,deflate`,
                'content-type': `application/json`,
            ...
            if (result.statusCode === 200) {
                ...
                if (responseBody.code === 200) {
                    const totalPoints = responseBody.data.scoreTotal;
                    DoubleLog(`🎯 积分总计: ${totalPoints}`);
                } else {
                    DoubleLog(`❌ 获取积分失败: ${responseBody.msg || '未知错误'}`);
                }
            } else {
                DoubleLog(`❌ 请求失败: HTTP ${result.statusCode}`);
            }
