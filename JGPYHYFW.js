const $ = new Env('交个朋友会员服务');
const JGPHY = ($.isNode() ? JSON.parse(process.env.JGPHY) : $.getjson("JGPHY")) || [];
let token = '';
let notice = '';
...
async function main() {
    ...
    for (const item of JGPHY) {
        id = item.id;
        token = item.token;
        console.log(`用户：${id}开始任务`);
        console.log('开始签到');
        let sign = await commonGet('/dtapi/pointsSign/user/sign');
        if (sign.code != 200) {
            await sendMsg(`用户：${id}\ntoken已过期，请重新获取`);
            continue;
        }
        ...
        console.log("————————————");
        console.log("查看直播预告");
        let viewLivePreview = await commonPost('/dtapi/task/complete', {"taskId": 3461});
        if (viewLivePreview.data.completed) {
            console.log(`查看成功，获得${viewLivePreview.data.points}积分`);
        } else {
            console.log('任务已完成');
        }
        ...
        console.log("————————————");
        console.log("订阅直播预告");
        let subscribeLivePreview = await commonPost('/dtapi/task/complete', {"taskId": 3462});
        if (subscribeLivePreview.data.completed) {
            console.log(`订阅成功，获得${subscribeLivePreview.data.points}积分`);
        } else {
            console.log('任务已完成');
        }
        ...
        console.log("————————————");
        console.log("社区发帖");
        let communityPost = await commonPost('/dtapi/task/complete', {"taskId": 3463});
        if (communityPost.data.completed) {
            console.log(`发帖成功，获得${communityPost.data.points}积分`);
        } else {
            console.log('任务已完成');
        }
        ...
        console.log("————————————");
        console.log("发布心愿");
        let publishWish = await commonPost('/dtapi/task/complete', {"taskId": 3458});
        if (publishWish.data.completed) {
            console.log(`发布成功，获得${publishWish.data.points}积分`);
        } else {
            console.log('任务已完成');
        }
        ...
        console.log("————————————");
        console.log("查询积分");
        let userCenterStats = await commonGet('/dtapi/user/points');
        console.log(`拥有积分：${userCenterStats.data.points}\n`);
        notice += `用户：${id} 拥有积分: ${userCenterStats.data.points}\n`;
    }
    if (notice) {
...
async function getCookie() {
    ...
    const index = JGPHY.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (JGPHY[index].token == newData.token) {
            return;
        } else {
            JGPHY[index] = newData;
...
async function commonPost(url, body) {
    return new Promise(resolve => {
        const options = {
            url: `https://smp-api.iyouke.com${url}`,
            headers : {
                ...
                'appId': 'wx78f8425753bf755f',
                'envVersion': 'release',
                'version': '2.3.24',
                'xy-extra-data': 'appid=wx78f8425753bf755f;version=2.3.24;envVersion=release;senceId=1005',
            },
