...
class UserInfo {
    ...
    async getUserInfo() {
        try {
            ...
            const userInfoResult = await httpRequest(userInfoRequest);

            if (userInfoResult.statusCode === 200) {
                const userInfoBody = JSON.parse(userInfoResult.body);
                if (userInfoBody.code === 0) {
                    const { score } = userInfoBody.data;
                    DoubleLog(`🎉 当前积分: ${score}`);
                } else {
                    DoubleLog(`❌ 获取积分信息失败: ${userInfoBody.message || '未知错误'}`);
                }
            } else {
                DoubleLog(`❌ 请求失败: HTTP ${userInfoResult.statusCode}`);
            }
        } catch (e) {
            console.log(e);
            DoubleLog(`❌ 获取积分信息失败: ${e.message}`);
        }
    }
}
