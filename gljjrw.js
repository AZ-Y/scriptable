...
******************************************/
const $ = new Env('囯乐酱酒');
const guoyuejiu = ($.isNode() ? JSON.parse(process.env.guoyuejiu) : $.getjson("guoyuejiu")) || [];
let Utils = undefined;
let notice = '';
...
async function getCookie() {
    ...
    const index = guoyuejiu.findIndex(e => e.authorization == newData.authorization);
    if (index !== -1) {
        if (guoyuejiu[index].authorization == newData.authorization) {
            console.log('Authorization未改变');
            return;
        } else {
            guoyuejiu[index] = newData;
            console.log('更新authorization:', newData.authorization);
            $.msg($.name, '更新authorization成功!', '');
        }
    } else {
        guoyuejiu.push(newData);
        console.log('新增authorization:', newData.authorization);
        $.msg($.name, '新增authorization成功!', '');
    }
    $.setjson(guoyuejiu, "guoyuejiu");
}
