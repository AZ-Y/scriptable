...
// icon-color: deep-blue; icon-glyph: cookie;
/**
 * ===================|调试区|====================
...
[Script]
JD-Cookie = type=http-request, pattern=^https?:\/\/api\.m\.jd\.com\/client\.action\?functionId=(wareBusiness|serverConfig|basicConfig), script-path=https://raw.githubusercontent.com/AZ-Y/scriptable/refs/heads/main/jd-cookie.js, requires-body=false

#Loon

http-request ^https?:\/\/api\.m\.jd\.com\/client\.action\?functionId=(wareBusiness|serverConfig|basicConfig) script-path=https://raw.githubusercontent.com/AZ-Y/scriptable/refs/heads/main/jd-cookie.js
...
const BOXJS_KEY = 'jdCookie'; // 在 BoxJS 中存储 Cookie 的键
$.body_key = BOXJS_KEY;
...
!(async () => {
  ...
  function GetCookie(request) {
    ...
    if (ptPinMatch && ptKeyMatch) {
      ...
      // 从存储中读取已有的 Cookie
      let savedCookie = $.getdata(`${BOXJS_KEY}_${pt_pin}`);
      ...
      if (savedCookie !== newCookie) {
        ...
        // 保存到设置中
        $.setdata(newCookie, `${BOXJS_KEY}_${pt_pin}`);
        console.log(`获取的 Cookie: ${newCookie}`);

        // 将新 Cookie 存入 BoxJS
        if (typeof $boxjs !== 'undefined') {
          $boxjs.setValue(`${BOXJS_KEY}_${pt_pin}`, newCookie); // 假设 BoxJS API 为 setValue
        }
      }
    } else {
