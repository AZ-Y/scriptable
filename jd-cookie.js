...
function GetCookie(request) {
  ...
  const ptPinMatch = cookie ? cookie.match(/pt_pin=(.*?);/) : null;
  const ptKeyMatch = cookie ? cookie.match(/pt_key=(.*?);/) : null;

  if (ptPinMatch && ptKeyMatch) {
    const pt_pin = ptPinMatch[1];
    const pt_key = ptKeyMatch[1];
    const newCookie = `pt_pin=${pt_pin}; pt_key=${pt_key};`;
    
    // 从存储中读取已有的 Cookie
    const savedCookie = $.getdata(`${BOXJS_KEY}_${pt_pin}`);
    
    // 如果新 Cookie 不同于已保存的 Cookie，则更新
    if (savedCookie !== newCookie) {
      const title = "Cookie 更新";
      const subtitle = `账号: ${pt_pin}`;
      const message = `新的Cookie: ${newCookie}`;
