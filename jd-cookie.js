  ...
  try {
    ...
    if (rawCookie) {
      ...
      if (ptPinMatch && ptKeyMatch) {
        ...
        let previousJdCookie = $.getdata('jdCookie');
        if (jdCookie !== previousJdCookie) {
          $.setdata(jdCookie, 'jdCookie');
          if (previousJdCookie) {
            $.msg(`JD Cookie 已更新`, ``, jdCookie);
          } else {
            $.msg(`新增 JD Cookie`, ``, jdCookie);
          }
        } else {
          $.log(`JD Cookie 未发生变化`);
        }
