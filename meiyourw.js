...
@Author: [Your Name]
@Date: [Date]
@Description: 美柚app签到脚本重构
------------------------------------------
*/

const $ = new Env("美柚app签到");
const ckName = "meiyou_data";
...
async function main() {
    for (let user of $.userList) {
        ...
        try {
            // 签到任务
            $.log(`[${user.userName || user.index}][INFO]开始签到...\n`);
            $.title = await user.signin() ?? "";
            if (user.ckStatus) {
                let userInfo = await user.userAccount();
                $.log(`[${user.userName || user.index}][INFO]查询用户信息成功...\n`);
                DoubleLog(`「${userInfo?.member_name}」当前共${userInfo?.coin}个积分`);
            } else {
...
class UserInfo {
    constructor(user) {
        ...
        this.ckStatus = true;
        this.baseUrl = 'https://coin-api.youzibuy.com';
        this.headers = {
    ...
    async userAccount() {
        try {
            let res = await this.fetch(`/api/account/getInfo/${this.userId}`);
            return res?.detail;
        } catch (e) {
            this.ckStatus = false;
