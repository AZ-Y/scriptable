...
青龙：
1. 登录后抓包 meiyou.com域名下的cookie，填写到meiyou_data，多账号用 @ 分割
2. 可选推送：将bark的key填写到bark_key，不填默认使用青龙自带的推送

Loon: 
1. 将获取Cookie脚本保存到本地
2. 打开美柚小程序->我的，若提示获取Cookie成功则可以使用该脚本
...
async function main() {
    ...
    for (let user of userList) {
        ...
        await user.signin();
        if (user.ckStatus) {
            await user.dailyTask1();
            await user.dailyTask2();
            await user.dailyTask3();
...
class UserInfo {
    ...
    async signin() {
        try {
            const options = {
                url: `https://api.meiyou.com/signin`,
                ...
                $.post(options, async (error, response, data) => {
                    try {
                        ...
                        if (result?.status === "success") {
                            ...
                            console.log(result);
                            console.log(`❌[${this.index}]签到失败！${result?.message}`);
                            this.ckStatus = false;
                        }
                    } catch (error) {
                        $.log(error);
    ...
    async dailyTask1() {
        let options = {
            url: `https://api.meiyou.com/daily/task1`,
            headers: this.headers,
        };
        return new Promise(resolve => {
            $.post(options, async (error, response, data) => {
                try {
                    ...
                    debug(result, '任务1信息');
                    if (result?.status === "success") {
                        DoubleLog(`✅任务1完成: ${result?.message}`);
                    } else {
                        DoubleLog(`❌任务1失败: ${result?.message}`);
                    }
                } catch (error) {
                    $.log(error);
                } finally {
    ...
    async dailyTask2() {
        let options = {
            url: `https://api.meiyou.com/daily/task2`,
            headers: this.headers,
        };
        return new Promise(resolve => {
            $.post(options, async (error, response, data) => {
                try {
                    ...
                    debug(result, '任务2信息');
                    if (result?.status === "success") {
                        DoubleLog(`✅任务2完成: ${result?.message}`);
                    } else {
                        DoubleLog(`❌任务2失败: ${result?.message}`);
                    }
                } catch (error) {
                    $.log(error);
                } finally {
    ...
    async dailyTask3() {
        let options = {
            url: `https://api.meiyou.com/daily/task3`,
            headers: this.headers,
        };
        return new Promise(resolve => {
            $.post(options, async (error, response, data) => {
                try {
                    ...
                    debug(result, '任务3信息');
                    if (result?.status === "success") {
                        DoubleLog(`✅任务3完成: ${result?.message}`);
                    } else {
                        DoubleLog(`❌任务3失败: ${result?.message}`);
                    }
                } catch (error) {
                    $.log(error);
                } finally {
...
async function getNotice() {
    try {
        const urls = ["https://raw.githubusercontent.com/YourName/Repo/main/notice.json", "https://raw.githubusercontent.com/YourName/Repo/main/tip.json"];
