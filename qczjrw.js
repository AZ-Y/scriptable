...
******************************************/
const $ = new Env('汽车之家');
const qczj = ($.isNode() ? JSON.parse(process.env.qczj) : $.getjson("qczj")) || [];
let Utils = undefined;
let notice = '';
...
async function main() {
    ...
    for (const item of qczj) {
        const authorization = item.authorization;
        console.log(`开始签到，authorization: ${authorization}`);
        let sign = await commonPost('https://lapp.api.autohome.com.cn/openapi2/cloud/bdp/autohome/usersignin/signin', {}, authorization);
...
async function getCookie() {
    ...
    if (index !== -1) {
        if (qczj[index].authorization == newData.authorization) {
