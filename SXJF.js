// env.js 全局
const $ = new Env("水星家纺签到");
const ckName = "shuixing_token";
//-------------------- 一般不动变量区域 -------------------------------------
...
async function main() {
    ...
    for (let user of userList) {
        if (user.ckStatus) {
            ...
            taskall.push(await user.queryPointsAndSignInDays());
            await $.wait(user.getRandomTime());
        } else {
            // 将ck过期消息存入消息数组
...
class UserInfo {
    ...
    async signin() {
        try {
            const url = `https://mi.shuixing.cn/wechat/mini/p/cs/mi/vp/interact/v1.0/signIn`;
            const method = `POST`;
            const headers = {
                'Accept-Encoding': `gzip,compress,br,deflate`,
                'content-type': `application/json`,
                'Connection': `keep-alive`,
                'Referer': `https://servicewechat.com/wx8000487d3402bdce/35/page-frame.html`,
                'Host': `mi.shuixing.cn`,
            ...
            const body = `{"signInDate":"${new Date().toISOString()}","storeId":123862}`;
            ...
            let result = await httpRequest(myRequest);
    ...
    // 查询签到天数和积分
    async queryPointsAndSignInDays() {
        try {
            const url = `https://mi.shuixing.cn/wechat/mini/p/cs/mi/vp/interact/v1.0/queryVpSignIn?signYear=2024&storeId=123862`;
            const method = `GET`;
            const headers = {
                'Accept-Encoding': `gzip,compress,br,deflate`,
                'content-type': `application/json`,
            ...
            if (result.statusCode === 200) {
                ...
                if (responseBody.code === 200) {
                    DoubleLog(`✅ 查询成功！签到天数: ${responseBody.data.signInDays}, 当前积分: ${responseBody.data.currentPoints}`);
                } else {
                    DoubleLog(`❌ 查询失败: ${responseBody.msg || '未知错误'}`);
                }
            } else {
                DoubleLog(`❌ 请求失败: HTTP ${result.statusCode}`);
            }
