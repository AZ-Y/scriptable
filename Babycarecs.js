...
async function getCookie() {
    ...
    if (index !== -1) {
        ...
        return; // 如果找到相同的 authorization，直接返回，不做任何操作
    } else {
        // 如果找到 authorization 但是不同，则更新
        const oldIndex = Babycare.findIndex(e => e.authorization.startsWith(authorization.substring(0, 30))); // 假设前 30 个字符不变
        if (oldIndex !== -1) {
            Babycare[oldIndex] = newData; // 更新现有的 authorization
            console.log('更新authorization:', newData.authorization);
            $.msg($.name, '更新authorization成功!', '');
        } else {
            Babycare.push(newData); // 否则，新增
