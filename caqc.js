const $ = new Env('长安汽车签到');
const Changan = ($.isNode() ? JSON.parse(process.env.Changan) : $.getjson("changan_data")) || [];
let Utils = undefined;
let notice = '';
...
async function main() {
    console.log('长安汽车签到开始');
    Utils = await loadUtils();
    for (const item of Changan) {
        const token = item.token;
        console.log(`开始签到，token: ${token}`);
        let sign = await commonPost('https://wxapi.uni.changan.com.cn/user/signIn', {"paramEncryptedStr": "SF6KqQbdNPPwX/dW8pj0Nw=="}, token);
        ...
        if (sign && sign.success) {
            ...
            switch (sign.code) {
                ...
                case 401:
                    ...
                    break;
                default:
                    failureMsg = sign.msg || '未知错误';
            }
            console.log(`签到失败: ${failureMsg}`);
            notice += `签到失败: ${failureMsg}\n`;
        }
    }
    if (notice) {
        $.msg($.name, '', notice);
