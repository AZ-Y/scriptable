...
const userNameKey = "asai_username";
const userPasswordKey = "asai_password";
...
let userName = ($.isNode() ? process.env[userNameKey] : $.getdata(userNameKey)) || '';
let userPassword = ($.isNode() ? process.env[userPasswordKey] : $.getdata(userPasswordKey)) || '';
let userList = [];
let userIdx = 0;
let userCount = 0;
...
async function main() {
    ...
    for (let user of userList) {
        ...
        if (user.ckStatus) {
            ...
            $.notifyMsg.push(`❌账号${user.index} >> Login failed!`);
        }
    }
}
...
class UserInfo {
    ...
    async login() {
        try {
            const options = {
                url: `https://audi2c.faw-vw.com/capi/v1/user/mobile_login`,
                headers: {
...
async function getUserInfo() {
    let usernames = userName.split(envSplitor);
    let passwords = userPassword.split(envSplitor);
    if (usernames.length !== passwords.length) {
        throw new Error("用户名和密码数量不一致，请检查环境变量配置。");
    }
    for (let i = 0; i < usernames.length; i++) {
        userList.push(new UserInfo(usernames[i], passwords[i]));
    }
...
async function httpRequest(options) {
    return new Promise((resolve, reject) => {
        ...
        $.http[method.toLowerCase()](url, { headers, body }, (err, resp, data) => {
            if (err) {
                reject(err);
            } else {
                try {
